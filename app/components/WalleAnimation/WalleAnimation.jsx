...
export default function WalleAnimation() {
  const [x, setX] = useState(0);
  const [y, setY] = useState(0);
  const [flip, setFlip] = useState(false);
  const [showMessage, setShowMessage] = useState(true);
  const audioPlayRef = useRef(false);
  const startArmsRef = useRef(null);
  const walleRef = useRef(null);
  ...
  const processInteraction = (clientX, clientY) => {
    const audio = new Audio('https://s3-us-west-2.amazonaws.com/s.cdpn.io/28963/Whoa.mp3');
    const walleBox = walleRef.current.getBoundingClientRect();
    ...
    if (clientX > walleCoords) {
      setX(-(clientX / 150));
      setFlip(true);
      if (!audioPlayRef.current) {
        audio.play();
        audioPlayRef.current = true;
      }
    } else {
      audioPlayRef.current = false;
      setX(clientX / 150 - 4);
      setFlip(false);

      gsap.set("#righteyeb2", {
        scaleX: 1 + (1 - clientX / walleCoords) / 4,
        ease: 'power1.out'
      });
      gsap.set("#lefteyeb2", {
        scaleX: 1 + (1 - clientX / walleCoords) / 4,
      ...
      gsap.set("#walle", {
        x: ((clientX / walleCoords) * 60) - 50,
        ease: 'power1.out'
      });

      startArmsRef.current.progress(1 - (clientX / walleCoords)).pause();
    }
  };
  ...
  return (
    <div 
      className="walle-container"
      onMouseMove={handleMove}
      onTouchMove={handleTouch}
    >
      {showMessage && (
        <p className="message-text">Move your mouse or tap to interact with WALL-E!</p>
      )}
      
      <svg 
        className="background-cog" 
        viewBox="0 0 200 200" 
        preserveAspectRatio="xMidYMid slice"
      >
        <path 
          d="M100,10v20m0,140v20m90-90h-20m-140,0H10m141.42-41.42l-14.14,14.14m-94.56,94.56l-14.14,14.14m14.14-94.56l14.14,14.14m94.56,94.56l14.14,14.14" 
          stroke="#fff" 
          strokeWidth="5" 
          opacity="0.3" 
        />
        <circle cx="100" cy="100" r="80" fill="none" stroke="#fff" strokeWidth="5" opacity="0.3" />
      </svg>
      ...
      >
        ...
        <g id="Layer_2">
          <g className={`head ${flip ? 'isFlipped' : ''}`}>
            <g id="neck">
              <g id="neckb">
                ...
                <g id="righteye">
                  ...
                  <g id="righteyeball">
                    ...
                    <g clipPath="url(#clip-path-3)">
                      <circle id="ball-2" cx="75" cy="60.2" r="4.3" fill="#394848" />
                    </g>
                    <circle id="glimmer-2" cx="74.7" cy="56.1" r="1.4" fill="#fff" />
                  </g>
                </g>
              </g>
            </g>
          ...
          <g className="body">
            ...
            <g id="back">
              ...
              <path fill="#a2b2b3" d="M76.4 72.1H86.3V80.6H76.4z" />
      ...
      <style jsx>{`
        ...
        :global(#leftarm), :global(#rightarm) {
