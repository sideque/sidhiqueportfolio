...
import OrbitImages from "@app/components/OrbitImages/OrbitImages";
...
export default function BackToTop({ id }: Props) {
  ...
  useEffect(() => {
    ...
    if (buttonRef.current) {
      gsap.fromTo(buttonRef.current,
        ...
        { scale: 1, rotation: 0, opacity: 1, duration: 0.8, ease: 'elastic.out(1, 0.5)', delay: 0.2 }
      );
    ...
    tl.add('redo')
      ...
      .fromTo('#lefteyeball',
        { scaleY: 1 },
        { scaleY: 0.2, duration: 0.1, repeat: 3, yoyo: true, transformOrigin: '50% 50%', ease: 'circ.out' },
        'redo+=3'
      )
      .fromTo('#righteyeball',
        { scaleY: 1 },
  ...
  return (
    <>
      <div
        className={`fixed right-8 bottom-8 sm:right-12 sm:bottom-12 lg:right-24 lg:bottom-16 z-[100] flex items-center justify-center ${backToTop}`}
        style={{ width: '120px', height: '120px' }}
      >
        {/* OrbitImages centered around the button */}
        ...
        {/* Button centered with full robot SVG */}
        <Button
          ref={buttonRef}
          ...
          variant="icon"
          size="max"
          className="relative p-2 bg-transparent hover:bg-white/10 dark:hover:bg-white/5 border-0"
          style={{ width: '120px', height: '120px' }}
        >
          <svg
            id="walle"
